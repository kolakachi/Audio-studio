...
var TextTip = function TextTip(config) {
  ...
  var savedSel = null;
  _defineProperty(this, 'savedSelection', null);
  ...
  _defineProperty(this, "_setupTooltip", function () {
    ...
    _this.config.buttons.forEach(function (btn, index) {
      ...
      if(btn.iconFormat == 'text') {
        btnEl.setAttribute('id', 'btn-text-count');
      }
      
      btnEl.style.transitionDelay = 40 * (index + 1) + 'ms';

      switch (btn.iconFormat) {
        case IconFormat.URL:
  ...
  _defineProperty(this, "_setupEvents", function () {
    ...
    document.getElementById("audio-textarea").addEventListener("mousedown", function(){
      _this._hide();
  ...
  _defineProperty(this, "_onSelectionChanged", function (event) {
    if (_this._selectionValid()) {
      ...
      _this.savedSelection= saveSelection();
      _this._show();
    } 
    // else {
  ...
  _defineProperty(this, "_onButtonClick", function (event) {
    ...
    var selection = window.getSelection();
    restoreSelection(_this.savedSelection);
    _this.config.buttons[btnIndex].callback(selection.toString(), btn);
